...
SELECT * FROM waiting_list; -- done
SELECT * FROM billing; -- done
SELECT * FROM reports;
SHOW TABLES;
DELETE FROM appointments WHERE patient_id IN (SELECT patient_id FROM patients);
DELETE FROM waiting_list WHERE patient_id IN (SELECT patient_id FROM patients);
DELETE FROM billing WHERE patient_id IN (SELECT patient_id FROM patients);

-- Now delete data from the patients table
DELETE FROM appointments;
-- describe waiting_list;
-- Optional: Reset the auto-increment counter for the patients table
...
ALTER TABLE appointments ADD COLUMN severity INT;
